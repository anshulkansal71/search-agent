...
class BasicToolsChat(ChatInterface):
    ...
    def initialize(self) -> None:
        ...
        self.response_prompts = {
            ...
            "definition_node": ChatPromptTemplate.from_messages(
                [("system", "should include examples and use cases"), ("user", "{query}")]
            ),
        }
    ...
    def process_message(self, message: str, chat_history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        def run_prompt(prompt: ChatPromptTemplate) -> str:
            ...
            return chain.invoke({"query": message}).strip()

        def classifier_node(state: Node) -> Node:
            return {"query_type": run_prompt(self.query_classifier_prompt)}
